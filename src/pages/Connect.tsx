    ...
    <div className="min-h-screen bg-background relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 py-20">
        ...
        <motion.div
          ...
          className="mb-16"
        >
          <p className="font-mono text-sm text-primary tracking-widest uppercase mb-3">{t("connect.label")}</p>
          <h1 className="text-4xl md:text-6xl font-bold mb-4">
            {t("connect.heading1")}<span className="text-gradient">{t("connect.heading2")}</span>
          </h1>
        ...
        <motion.div
          ...
          className="mb-16"
        >
          <h2 className="text-2xl font-bold mb-6">{t("connect.sendMessage")}</h2>
          <div className="grid md:grid-cols-5 gap-8">
            <div className="md:col-span-2 space-y-5">
              <div className="glass rounded-xl p-6 flex items-start gap-4">
                <Mail className="w-5 h-5 text-primary mt-0.5" />
                <div>
                  <p className="font-semibold text-sm">{t("connect.email")}</p>
              ...
              <div className="glass rounded-xl p-6 flex items-start gap-4">
                ...
                <div>
                  <p className="font-semibold text-sm">{t("connect.location")}</p>
            ...
            <form onSubmit={handleSubmit} className="md:col-span-3 glass rounded-xl p-8 space-y-5">
              <input
                type="text"
                placeholder={t("connect.yourName")}
                value={formData.name}
              ...
              <input
                type="email"
                placeholder={t("connect.yourEmail")}
                value={formData.email}
                onChange={(e) => setFormData({ ...formData, email: e.target.value })}
                className="w-full px-4 py-3 rounded-lg bg-secondary border border-border text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all text-sm"
              />
              <textarea
                placeholder={t("connect.yourMessage")}
                rows={4}
              ...
              >
                {t("connect.sendBtn")} <Send className="w-4 h-4" />
              </button>
            </form>
          </div>
        </motion.div>
        ...
        >
          <h2 className="text-2xl font-bold mb-2">{t("connect.resumeTitle")}</h2>
          <p className="text-muted-foreground mb-6">{t("connect.resumeDesc")}</p>
          <a
